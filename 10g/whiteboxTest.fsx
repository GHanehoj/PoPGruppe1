...
brd.insert wlf
brd.insert moose
printfn "brd.getContent, forventer output [wlf; moose] eller [moose; wlf]: %A" (brd.getContent = [wlf; moose] || brd.getContent = [moose; wlf])

printfn ""
printfn "Test af Moose: "
printfn "Test af moose.nearbyWolves(): "
printfn "Forventer Seq.head (moose.nearbyWolves) = wlf.pos: %A" (Seq.head (moose.nearbyWolves ()) = (wlf.pos))
...
do moose.takeTurn ()
printfn "Forventer nu at mooose.distanceTo (moose.pos) (wlf.pos) = 2: %A" ((moose.distanceTo (moose.pos) (wlf.pos)) = 2)


printfn ""
printfn "Test af Wolf: "
printfn "Test af wlf.nearbyMoose(): "
printfn "Forventer Seq.head (wlf.nearbyMoose ()) = moose.pos: %A" (Seq.head (wlf.nearbyMoose ()) = (moose.pos)) 

printfn ""
printfn "Test af wlf.takeTurn():"
printfn "Forventer et kald til wlf.huntMoose hvilket burde sætte wlf.pos = (3,3), da moose.pos = (4,4)."
do wlf.takeTurn ()
printfn "Forventer wlf.pos = (3,3): %A" (wlf.pos = (3,3))

printfn ""
printfn "Test af moose.die(): "
printfn "Der kaldes nu igen wlf.takeTurn() og der forventes nu under prioritize at Eat(4,4) vælges."
do wlf.takeTurn ()
