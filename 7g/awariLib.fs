...
// Hjælpefunktion til at bestemme om et 
// bestemt felt er en bestemt spillers hjemmefelt
let isHome (p:player) (i:pit) : bool = 
    let pitPlayer = if i.index < 7 then Player1 else Player2
    (pitPlayer = p) && (i.index%7=6)

// Hjælpefunktion til at bestemme om en brætkonfiguration 
// medfører at spillet er slut
...
// Hjæplefunktion til at erstatte feltet 
// i et bestemt index med et givent felt.
...
// Opdaterer det sidste felt, hvis dette felt kun 
// indeholder 1 bønne, dvs. distribute endte i et tomt felt
...
let updateLastPit (b:board) (p:player) (i:pit) : (board*pit) = 
    if i.beanCount = 1 && i.index % 7 <> 6 then
        ...
        let newHomePitCount = home.beanCount + i.beanCount + oppositePit.beanCount 
        let updatedBoard =
            b 
            |> replaceAtIndex home.index {home with beanCount = newHomePitCount}
...
// Hovedfunktionen til at uddele bønnerne på et 
// specifikt felt ud til alle de efterfølgende felter, 
// samt fjerne de oprindelige bønder i startfeltet.
let distribute (b:board) (p:player) (i:pit) : (board*pit) =
    
    // Hjælpefunktion til at bestemme hvordan 
    // bønnerne i et givent felt cp ændre sig efter
    ...
    let addBean (op:pit) (cp:pit) : pit =
        ...
        // Skulle det valgte felt indeholde nok bønder 
        // til at komme hele vejen rundt adderes 
        // dette tal til alle felter.
        // 
        // Herudover sættes op til nu at have 0 bønner
        let rounds = op.beanCount / boardSize
        let beanSum = (if cp <> op then cp.beanCount else 0) + rounds
